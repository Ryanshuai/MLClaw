...
When a tier requires actually launching the user's code (Tier 1+ smoke / mini / full benchmark), follow `lifecycle/references/run-mechanics.md` "Run Skill Internal Dependencies" Step 2 (`code_snapshot.py` for SHA + dirty patch) and Step 3 (`cwd` = unified code_dir, `output_dir` = absolute under `<RUN_DIR>/output/`). Refactor-run is the exception in that the code_dir is `stages/refactor/code/` directly (no `_source` symlink — refactor edits in place); the cwd / output_dir / snapshot rules still apply.
...
Also set `run.json -> mode` / `scope` on the verification run: Tier 3 → `mode: "debug"` with the subset in `scope`; Tier 4 → `mode: "production"`. This keeps mini numbers out of every downstream comparison automatically (`lifecycle/references/run-mechanics.md` "Metric comparability").
