...
- **`spec_version`** — the guideline the labels were produced under. Two batches under different specs are different distributions, and merging them silently degrades the model. This is the `preprocessing` mismatch rule (`lifecycle/references/run-mechanics.md` "Preprocessing contract (cross-stage)") applied to labels instead of pixels: when a stage draws on more than one handoff, diff the spec versions and surface any difference **before** the run, not after a confusing metric three weeks later.
