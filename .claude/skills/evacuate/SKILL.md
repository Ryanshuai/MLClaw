...
## 存成什么形状:ARA 的层 + 一层 ARA 没有的

工件结构照 ARA(arXiv:2604.24658)。**输入是 `src/`,输出是其余四层。**

| 层 | 装什么 | 为什么是这层 |
|---|---|---|
| `src/` | code snapshot、`config_snapshot.json`、`sources.json`、env | **搞架构搜索时代码就是自变量**,所以这层不是背景,它**就是可复现性本身** |
| `evidence/` | `stream.jsonl`、指标、`tb/`、**原始日志** | 日志在这层而不是单开一层:MLClaw 的接地规矩要求数字引到抄下来的那一行,**那一行就是 evidence**,同一个词,低一层 |
| `logic/` | `knowledge/conclusions.json` | `/conclude` 的产出 —— 现在相信什么、凭什么、什么会推翻它。ARA 的 `logic/claims.md` |
| `trace/` | `graph.json`、`findings.json`、`baseline.json` | `/explore` 的实验图。**这层决定了一年后那次 ablation 还读不读得懂**,还是只剩一堆 run 目录 |
| **`weights/`** | `.pth` / `.ckpt` / `.safetensors` | **‼️ ARA 没有这层**,而且不是疏漏:论文的产出是知识,知识能从 src+evidence 重新长出来,**4GB 的 checkpoint 不能**。它是五层里唯一不可重建的,所以也是传一半代价最大的 |
| `unclassified` | 规则没认出来的 | **照搬不误**,只是报一声。只留认得出的东西的清扫,正是「谁都没想到的那个文件」丢掉的方式,而且它一边丢一边报成功 |

`bundle` 会写一份 `ARTIFACT.md` —— ARA 的 `PAPER.md` 那一页:输入是什么、出来了什么、
**两者还连不连得上**。`logic/` 和 `trace/` 会**实体拷进 bundle**,因为它们必须在
「不下载四十个 G 的权重」的前提下就能读。
