...
- [x] Data archaeology — `/discover` (inherit a project and find out what data exists: sweep code, git history, tracking, S3, docs, people; `gone` and `unreachable` are never the same answer). Called by the `-init` skills
...
- [x] Research artifact — `/ara` (a round's work in ARA's four layers plus `weights`, an `ARTIFACT.md` a person can read a year later, and a check that reports where the frozen beliefs stopped agreeing with the live record)
...
- [x] Machine register — `/roll-call` (whose box is this and do they still need it: four states — `held` / `claimed` / `attributed` / `unaccounted` — where an orphan list used to hold the last three, because the tag proves *MLClaw* made a box and not that **this ledger** did. A registration is a claim and can never become verified; the lifecycle log is the only independent evidence and it answers who *created* it, not who needs it. Probes before it asks a person, and releases nothing)
