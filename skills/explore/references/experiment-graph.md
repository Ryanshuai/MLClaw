...
| ‼️ `varies` | what this arm CHANGES against its parent, **written the way it is switched** (`--sku_ce (decoder matching branch only)`). This is the expansion of `run_id` | `run_id` is a name (`sm`, `sl1`, `e13qh`) and the name's meaning lives only in the session that coined it. One compaction later, the arm is still running and the only remaining answer is a guess — the recorded round spent about a third of the user's turns re-asking what eight arm names stood for |
...
| ‼️ **Every open 🔵/🟪 has a `varies`** (`arm_without_legend`) | MLClaw's addition. Major, not critical: the arm is running correctly and it is the RECORD that is broken — which is why it breaks silently. `status` prints `varies` beside `run_id` for every open arm, along with `serves` (the parent question) and `since` (derived from `history`, never stored twice). ‼️ `serves` reading null is drift's shape: an arm running for no registered question. In the recorded round that was an lr sweep and a depth line opening while the registered question was where SKU gets injected, and the user cut both by hand |
