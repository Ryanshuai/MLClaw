...
**`<mlclaw_root>` is resolved, never assumed** — `python <repo>/scripts/shared/workspaces.py tool` prints it, self-bootstrapping from `__file__` and caching in `~/.mlclaw/state.json` (`references/layout.md` -> "Workspace and tool-repo location"). ‼️ A bare `python scripts/…` is only correct when the working directory happens to be this repo, and when it is not, **nothing raises**: the fallback rule below turns a missing script into "do the same work manually", so a `retention.py` refusal, a `graph.py check` or an `evacuate.py clearance` silently stops running while the flow still reads as working. `contract_docs.ScriptPathsAreResolvedNotAssumed` holds the line.
