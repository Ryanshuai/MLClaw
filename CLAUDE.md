...
- **JSON configs are the source of truth**: fixed keys, you fill values. Templates at `template/` top level and `template/<link>/`, filled instances in user projects. **One folder per link, the folders flat** — no folder contains another, and `template/meta.json` carries the relationships the tree cannot state: which folder is a stage, and how each file reaches a project (`stage-config` copied at init · `record` instantiated per run/session/round · `project` · `workspace` · `reference`). ‼️ **It is not "the set `/project-init` copies"** — that sentence stood here and was wrong: `PROJECT_TEMPLATES` is one file, and `run.json` is never copied at all, it is read at every run creation. Nor is it a lifecycle: **order is `<mlclaw_root>/references/skill-graph.md`'s**, and nothing in `template/` may restate it. The tool's own halves sit at the repo root beside `skills/`, where the plugin spec puts them: `scripts/` ("Helper scripts and utilities") and `references/` ("documentation intended to be loaded into context as needed"). Nothing at the root is copied into a project.
...
- **Never let a value have two authors.** A constant, a vocabulary or a table that two places must agree on is written **once**, and everywhere else imports it — including in prose, where a copy is the one no test executes. Nothing raises when the copies drift, because each side stays internally consistent; the reader simply stops recognising a value the writer now emits, and returns an answer that is legal and wrong. Measured in one pass: the machine shape had **three** authors inside `pool.py` alone, so a dimension added to L2's table would not reach the wire and the search rents a box that does not meet the constraint; `TIERS` had two, and CLAUDE.md's own rule is that *the tier travels with the number forever*; three atomic writers had drifted three ways, leaving the money ledger as the one file written with non-ASCII escaped. ‼️ **A copy that says it matches is the copy that stops matching without telling anyone** — `pool.py` carried its own `DEFAULT_TTL_S` under the comment "matches L2's default". Enforced by `contracts/contract_one_author.py`; the scan that finds new ones is `/agent-refactor` Step 5b.
...
- **Never repeat a conclusion without re-reading its status.** "We tried multi-frame fusion, it did not help" is a sentence about a corpus, a tier and a noise floor, and it carries none of them — which is why it goes on being applied to corpora it was never measured on, and why nothing raises when the run behind it was deleted last month. `knowledge/conclusions.json` holds the three qualifiers and `conclude.py status` re-derives what the evidence *currently* supports; a belief quoted from memory is a claim wearing a conclusion's clothes. ‼️ The state that actually occurs is **`unverifiable`** — the evidence stopped resolving — and it is neither a weak `supported` nor a `refuted`: nobody can check, which is the third fact, exactly as with a census that could not reach a machine.
- **Never quote a number without the tier it was measured at.** A `[T1 trend]` conclusion cited next week as a controlled one is how a soft number becomes a hard one, and it is how a false noise floor entered a record once already. The tier travels with the number forever, in every file and every sentence. Its companion: **with no measured noise floor, "no significant improvement" is undecidable, not negative** — three facts, not two, exactly as with a census that could not reach a machine. ‼️ And the floor itself has four states, not two: a floor measured before MLClaw arrived — on a box since released, from a checkpoint on a powered-down disk — is written `origin: external` and is a `claim`; it gates T2 and can never carry a T3. **Never satisfy a check with a reference that does not resolve.** Naming two run ids nothing can read, so that a floor passes, is the shape this closes: for one release the record layer refused the honest form and let the invented one through, which is a check paying for the thing it exists to stop.
...
| Before you… | Read |
|---|---|
| **run any skill** — check its requirements, offer the next one, push/pop state, or write any record | `<mlclaw_root>/references/skill-graph.md` — node hierarchy, the requires/suggests table, requirement checks, workflow state protocol |
| execute or finalize a run (`/train-run`, `/eval-run`, `/infer-run`, `/refactor-run`) | `<mlclaw_root>/references/run-mechanics.md` — step chain, launch contract, asset resolution, code snapshot, record integrity, checkpoint selection, retention, listing runs, path mapping |
| run `/explore` — propose, port, ablate, or adjudicate an architecture change | `skills/explore/references/experiment-graph.md` — node schema, the seven states, the four operations, **the working tree each arm is written in** (§1.5 — parallel arms share one by construction, and a contaminated snapshot reproduces exactly), and what `graph.py check` enforces |
| **decide whether a search is `/train-tune`'s or `/explore`'s** — the user asked to "search" / "搜一下" and it is not obvious which | `<mlclaw_root>/references/skill-graph.md` — "`/train-tune` vs `/explore`". The test is not params-vs-code, and getting it backwards costs a whole tune session |
| **hold more than one machine at a time** — `/train-tune` with `max_concurrent > 1`, any search that rents rather than borrows | `<mlclaw_root>/references/fleet.md` — slots vs machines, owned-before-rented, what a partial sweep is worth, preemptible as the search default, and why a preempted trial must never be read as a refuted hypothesis |
| find code/data on disk, resolve a `${}` reference, or create a project | `<mlclaw_root>/references/layout.md` — workspace and tool-repo location, code-source resolution, full file layout, dataset identity and census records, `${}` syntax |
| work the data line — freeze, curate, retire, or route a dataset | `<mlclaw_root>/references/data-line.md` — what each phase owns, what is not a phase, how `/data` composes them |
| pick up something designed but not built | `docs/roadmap.md` — and note that nothing in it has a script to call |
| **argue about an id** — mint one, scope one, or decide what a name may resolve to | `docs/identity.md` — 4 + 3 + 4 kinds and zero machine ids, adjudicated against the code. A settled debate, not a contract: it is in `docs/` for that reason, and the normative half lives in `<mlclaw_root>/references/layout.md` |
| change a script, a template, or a contract | `contracts/` and "Contracts" below |

## Status

**Implemented**: inference (init + run), evaluation (init + run + report + triage), refactor (init + run + report), training (init + run + tune + tune-report), **exploration** (`/explore` — architecture search, ported from the `arch-transplant` skill) with **`/conclude`** closing it (the belief layer: what is now believed, on what evidence, and what would overturn it — borrowed structurally from ARA's `logic/claims.md`), project init, resources, lease, **`/evacuate`** (empty a machine before it is released, prove every byte arrived, and store the result as an ARA-shaped artifact — `src` code+config, `evidence` numbers, `logic` conclusions, `trace` the ablation graph, `weights`), handoff, reproduction, and the whole data line (collect + label + curate + freeze + retire, plus check / audit / route / report / online-sample). **Multi-machine is real** as of the fleet layer: two rented-compute adapters (`nebius`, `lambda`) beside the owned-hardware one, and `scripts/shared/pool.py` holding N slots for one search, which `/train-tune` calls. What is *not* there: **multi-node distributed training** — a slot is one machine — and no bin-packing across concurrent searches. Full list: `<mlclaw_root>/references/fleet.md` "What this does not do".

**Shipped as a plugin**, and the repo root is the plugin root — `.claude-plugin/` holds both the
manifest and the catalog, `skills/` `agents/` `hooks/` `scripts/` `references/` sit beside `template/` — every one of those a directory the plugin spec names. That layout is not
...
`<mlclaw_root>/references/data-line.md`. Every rule there is cited by a check.
...
| `/explore` | **Architecture search** — turn "the results are bad / the architecture is primitive" into numbered, pre-registered, controlled changes: count the phenomenon, audit your own code in four states (**most "missing" techniques are present and defeated by one line**), price with existing flags, then port one flag at a time and ablate. **The model is not settled, and settling it is the work**: structure, components, and network selection — which model this should be, not how to configure the one you have. Its unit is a **proposal**, not a point in a config. It searches parameters too, when the parameter *is* the hypothesis ("maybe it just does not have the capacity") or when a ported component needs a fair operating point — but each such search is a card with its own pre-registered criterion, never a free axis mixed into an architecture arm, and the control arm gets the same budget. `/train-tune` is the same layer one step later: `/explore` decides what to tune, `/train-tune` tunes it. The dividing test, and the three shapes of parameter search: `<mlclaw_root>/references/skill-graph.md` -> "`/train-tune` vs `/explore`". Executes nothing: an arm is an ordinary run. Its record is `graph.json`, and `graph.py check` is the thing the source design asked for and never had — **its invariants reported, never repaired** (no count here on purpose — the one that was here said nine while the code emitted twenty-one) |
| `/lease` | Acquire, renew and release a rented compute host, with a dead-man switch so nothing is left running. Also answers "what am I paying for right now" — and **"did I actually release that one"**, which no list can answer: absence from an inventory is equally consistent with a scope nobody enumerated, so only `history`'s lifecycle record proves release. Reports **two meters**: compute stops on its own, storage does not — it starts at create, survives the box, and is the half a sweep of instances cannot see. Adapters: `ssh` (owned), `nebius` and `lambda` (rented), and **they disagree usefully** — live price vs hand-written claim, an audit log vs none, a guest-side dead-man switch vs one that is *worse* than absent. **Holding several at once is `pool.py`'s**, above this layer — `<mlclaw_root>/references/fleet.md` |
| `/roll-call` | **Whose machine is this, and do they still need it** — the question `reap` cannot answer and that a kill decision rests on. Registers what a box is FOR (`lease.py claim`: **a claim, never a verified fact**), joins it against the provider's lifecycle log, and sorts every swept resource into **four** states — `held` / `claimed` / `attributed` / `unaccounted`. ‼️ Only the first is yours: an orphan list used to hold the other three, and `TAG_PREFIX` proves *MLClaw* made a box, not that **this ledger** did — so on a shared tenant the conversation-start `reap` reported a colleague's live training box as forgotten. `unaccounted` has four causes (`not_attributed` = nobody asked · unsupported · looked-and-didn't-find · the log timed out) and none of them means unowned. Probes before it asks anybody anything, files what is left through `/ask-human` with the probe as `--verify`, and **releases nothing, ever** — that is `/evacuate` then `/lease release`. `whose` answers both directions from the ledger alone, with no network, which is what keeps "which machines did that search use" answerable after every box is gone |
...
5. **Check whether anything is still billing**: gated on `{WORKSPACE}/resources.json -> compute` having a non-`_` key (a free local read — `{WORKSPACE}` is `project.json -> workspace`). No rented provider registered → **skip silently**, which is most projects and costs nothing.
   - `python <mlclaw_root>/scripts/lease/lease.py --resources {WORKSPACE}/resources.json reap --billing-only`
   - **This is the only step here that goes to the network, and `--billing-only` is what buys it.** Step 4 refuses to `scan` unprompted because four ssh timeouts is not a greeting — and that reasoning holds: without the flag this sweeps `provider_ssh` too, ssh'ing every registered server before the user has said a word. With it, only providers that can bill are asked. What it protects is different in kind from a stale census: a census that is a week old costs nothing and can be re-scanned tomorrow, while a forgotten box **bills every hour and the meter only goes up**. It is also the one check whose local record cannot be trusted by construction — the session that opened the fleet is the one that died — which is why nothing local can gate it further.
   - **Read `complete` before quoting the count.** `orphans: []` off a sweep that reached nothing is byte-identical to "there are none"; `orphans_is_lower_bound` is the only thing separating them. Same rule as the census, and the same rule as *Never report data you could not look at*.
   - **Report `compute_without_adapter` first when it is non-null, ahead of the count.** It names a `compute` block whose key matches no installed adapter — a provider the user believes is registered and **nothing will ever sweep**. Every session will otherwise report "nothing is billing" about an account that is, and the report will be honest about everything it looked at.
   - Report **`orphan_storage` beside `orphans`**, never folded in. Compute stops on its own; storage starts at create, survives the instance being deleted, and is the half a sweep of instances cannot see. `total_is_lower_bound` means rows went unpriced — say so rather than presenting the figure as a bill.
   - **`orphans` is a kill list, so read what is beside it.** `reap` sorts every swept row into
     four states and only `held` is yours; the other three are reported separately (`claimed`,
     `attributed_to_others`, `unaccounted`) and **none of them may be proposed for teardown here**.
     ‼️ The tag proves *MLClaw* made a box, not that **this ledger** did — two people running
     MLClaw against one tenant produce boxes nothing can tell apart by prefix — so the narrow,
     automatic sweep is exactly where a colleague's live training box used to appear as forgotten.
     `attribution: {"asked": false}` means the ownership join was never run, which is the default
     here: report it that way rather than as "everything is ours". Whose a box is, and whether its
     holder still needs it, is `/roll-call`'s and is never automatic.
   - Report and offer to release. **Never release unprompted** — that is an irreversible action on a machine whose disk may be the only copy of a run, and `/evacuate` comes first. *Never release a machine you did not evacuate.*

Steps 1, 3 and 4 all scan the project; do them in one pass. Step 5 is separate: it is workspace-scoped rather than project-scoped, and it is the only one that leaves the machine.
...
**What is enforced**: the record layer only — `<mlclaw_root>/references/run-mechanics.md` "Record integrity". Everything else in this file is a contract nothing checks; when you touch one of those, you are the check.
