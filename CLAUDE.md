...
**Implemented**: inference (init + run), evaluation (init + run + report + triage), refactor (init + run + report), training (init + run + tune + tune-report), **exploration** (`/explore` — architecture search, ported from the `arch-transplant` skill), project init, resources, lease, handoff, reproduction, and the whole data line (collect + label + curate + freeze + retire, plus check / audit / route / report / online-sample). **Multi-machine is real** as of the fleet layer: a rented-compute adapter beside the owned-hardware one, and `lifecycle/scripts/shared/pool.py` holding N slots for one search, which `/train-tune` calls. What is *not* there: **multi-node distributed training** — a slot is one machine — and no bin-packing across concurrent searches. Full list: `lifecycle/references/fleet.md` "What this does not do".
...
| `/explore` | **Architecture search** — turn "the results are bad / the architecture is primitive" into numbered, pre-registered, controlled changes: count the phenomenon, audit your own code in four states (**most "missing" techniques are present and defeated by one line**), price with existing flags, then port one flag at a time and ablate. Its unit is a **proposal**, not a trial — `/train-tune` is the same layer with a different unit. Executes nothing: an arm is an ordinary run. Its record is `graph.json`, and `graph.py check` is the thing the source design asked for and never had — **nine invariants, reported, never repaired** |
| `/lease` | Acquire, renew and release a rented compute host, with a dead-man switch so nothing is left running. Also answers "what am I paying for right now" — and **"did I actually release that one"**, which no list can answer: absence from an inventory is equally consistent with a scope nobody enumerated, so only `history`'s lifecycle record proves release. Adapters: `ssh` (owned) and `nebius` (rented). **Holding several at once is `pool.py`'s**, above this layer — `lifecycle/references/fleet.md` |
