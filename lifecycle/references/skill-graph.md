...
| a number is verified rather than claimed | a **closed** `repro/*/session.json` whose `verdict` is `reproduced` or `reproduced_with_drift`. The only thing that moves an inherited checkpoint's `origin.confidence` off `claimed`; a session left open is not it |
