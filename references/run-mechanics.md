...
| A **production** run launches only on data whose latest audit is `clean` — and `never_audited`, `unverifiable` (a `SKIP` or missing layer), `stale` and `unreadable` are each refusals, not one. | `/data-audit` exists to find the wrong label *before* a run, where `/eval-triage` can only find it *through* one. It does — and then nothing stopped the run: the audit writes `[FATAL] compatibility: class list has 81 entries, model config says 80`, and training launches on ids the dataloader silently clamps. The four non-clean states must not collapse: `/data-audit` Step 7's own rule is that *an audit missing its compatibility section reads identically to one that passed it*, so a `SKIP` is the `never_audited` fact per layer, and a verdict about an older snapshot is about different bytes. Same three-fact split `census.py` draws between a machine that did not answer, a path that is not there, and a directory genuinely empty. Enforced by `audit_gate.py`. |
